...
echo "$($PSQL "SELECT w.name FROM games AS g FULL JOIN teams AS w ON g.winner_id = w.team_id FULL JOIN teams AS o ON g.opponent_id = o.team_id WHERE year=2014 AND round = 'Eighth-Final' UNION SELECT o.name FROM games AS g FULL JOIN teams AS w ON g.winner_id = w.team_id FULL JOIN teams AS
 o ON g.opponent_id = o.team_id WHERE year=2014 AND round = 'Eighth-Final'")"
